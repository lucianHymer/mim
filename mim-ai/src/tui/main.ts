...
interface GameState {
  ...
  agentDone: boolean;
  /** Loaded answered reviews for multi-turn processing */
  answeredReviews: AnsweredReview[];
  ...
  /** Bridge Approach phase: 'walking' | 'dead' | 'retreat' */
  bridgeApproachPhase: 'walking' | 'dead' | 'retreat';
  /** Whether player has seen the signpost */
  hasSeenSign: boolean;
  ...
  /** Wellspring input mode: INSERT for typing, SCROLL for navigation */
  wellspringMode: 'INSERT' | 'SCROLL';
  /** Current input buffer for wellspring text entry */
  ...
  /** Recent tools that have been executed */
  recentTools: string[];
  /** Count of tools executed since last message */
  toolCountSinceLastMessage: number;
  /** Whether to show the tool indicator */
 ...
 * Follows Arbiter's change detection pattern - track all state that affects rendering
 */
interface RedrawTracker {
  ...
  // Title screen change detection
  titleDrawn: boolean;

  // Bridge Approach change detection
  baLastPlayerCol: number;
  baLastPlayerRow: number;
  baLastPhase: string;
  baLastShowDialogue: boolean;

  // Bridge Guardian change detection
  bgLastHumanCol: number;
  bgLastHumanRow: number;
  bgLastGuardianCol: number;
  bgLastGuardianRow: number;
  bgLastShowModal: boolean;
  bgLastPhase: string;
  bgLastReviewIndex: number;
 ...
 */
function createBridgeApproachCollisionMap(): number[][] {
  const map: number[][] = [];
 ...
 */
function baIsWalkable(x: number, y: number): boolean {
 ...
 */
function baIsDeathZone(x: number, y: number): boolean {
...
class MimGame {
   ...
   */
  private async animateGuardianHopBetweenQuestions(): Promise<void> {
  ...
  private async setupBridgeApproachScene(): Promise<void> {
   ...
   */
  private async animateBridgeGuardianEntrance(): Promise<void> {
   ...
   */
  private wellspringCanUseTool = async (tool: string, input: Record<string, unknown>): Promise<{ behavior: 'allow'; updatedInput: Record<string, unknown> } | { behavior: 'deny'; message: string }> => {
   ...
   */
  private async processWellspringEvents(
    session: AsyncIterable<{ type: string; subtype?: string; session_id?: string; message?: { content?: Array<{ type: string; text?: string }> }; structured_output?: unknown; errors?: string[] }>
  ): Promise<void> {
    const AGENT = AGENTS.WELLSPRING;
  ...
  private async runWellspringAgent(): Promise<void> {
    ...
    const reviewsText = this.state.answeredReviews.map(r =>
      `Review ${r.id}:
      Question: ${r.question}
      Answer: ${r.answer}
   ...
   */
  async sendWellspringMessage(text: string): Promise<void> {
    const AGENT = AGENTS.WELLSPRING;
  ...
  private handleTitleInput(key: string): void {
  ...
  private handleBridgeApproachInput(key: string): void {
   ...
   */
  private drawBridgeApproachScene(): void {
    if (!this.state.tileset) return;
    ...
    if (
      playerCol === this.tracker.baLastPlayerCol &&
   ...
   */
  private drawSignpostDialogue(sceneOffsetX: number, sceneOffsetY: number, sceneWidthChars: number): void {
    if (!this.state.tileset) return;
   ...
   */
  private createMiddleFill(leftTile: ReturnType<typeof extractTile>, charRow: number): string {
   ...
   */
  private wrapTextWithBg(text: string, bgColor: string): string {
   ...
   */
  private drawBridgeApproachDeathScreen(layout: ReturnType<typeof getLayout>): void {
    if (!this.state.tileset) return;
   ...
   */
  private drawBridgeApproachRetreatScreen(layout: ReturnType<typeof getLayout>): void {
    if (!this.state.tileset) return;
   ...
   */
  private drawBridgeGuardianScene(): void {
    if (!this.state.tileset) return;
    ...
    const guardianAnimFrame = this.guardianSprite?.animation?.type === 'hopping'
      ? (this.guardianSprite.animation as { frame: number }).frame
      : -1;
    ...
    const bufferLine = (x: number, y: number, content: string) => {
   ...
   */
  private buildQuestionModalBuffer(sceneOffsetX: number, sceneOffsetY: number, sceneWidthChars: number): string {
    if (!this.state.tileset) return '';
  ...
  private createMiddleRowBorders(
    topLeftTile: ReturnType<typeof extractTile>,
    topRightTile: ReturnType<typeof extractTile>,
    charRow: number,
