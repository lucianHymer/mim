...
function renderGradientArt() {
    ...
    for (let row = 0; row < TITLE_ART.length; row++) {
        ...
        for (let col = 0; col < line.length; col++) {
            ...
            const colorIdx = Math.min(Math.floor((diagonal / maxDiagonal) * MIM_COLORS.length), MIM_COLORS.length - 1);
